...
public class Task05 {
    ...
    public static void main(String[] args) {
        ...
        Map<Character,Integer>harfSay=new TreeMap<>();
        for (int i = 0; i <sentence.length() ; i++) {
           char c=sentence.charAt(i);
           if (harfSay.containsKey(c)){
               harfSay.replace(c, harfSay.get(c)+1);
           }else harfSay.put(c,1);


       }
        for (  Map.Entry<Character,Integer> entry: harfSay.entrySet()) {
            System.out.println("Karakterinden "+entry.getKey()+" "+entry.getValue()+" tane var...");
        }
    }
