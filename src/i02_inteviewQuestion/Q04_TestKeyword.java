...
*/
    ...
    private static void testSonucu(char[][] answers, char[] keys) {
        for (int i = 0; i <answers.length ; i++) {
            int counter=0;//herbir ogrencinin dogru cvp sayısı
            for (int j = 0; j <answers[i].length ; j++) {// herbir ogncnin cvplarına bakacak
                if(answers[i][j]==keys[j]) {// ogrencinin cevabıyla cvp anahtarını karşılaştırma
                    counter++;
                }
            }
